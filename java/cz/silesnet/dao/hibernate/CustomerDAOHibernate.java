...
public class CustomerDAOHibernate extends HibernateDaoSupport implements
	...
	public List<Customer> getByExample(Customer cExample) {
		...
		SearchUtils.addIlikeRestriction(crit,
				"substring(replace(public_id, '/', ''), 1, 8)", cExample
						.getExportPublicId());
		SearchUtils.addIlikeInRestriction(crit, "contract_no", cExample
				.getContractNo());
		SearchUtils.addEqRestriction(crit, "symbol", cExample.getSymbol());
		// contact restrictions
		if (cExample.getContact() != null) {
			SearchUtils.addIlikeRestriction(crit, "email", cExample
					.getContact().getEmail());
			// address restrictions
			if (cExample.getContact().getAddress() != null) {
				Address address = cExample.getContact().getAddress();
				SearchUtils.addIlikeRestrictionI18n(crit, "street", address
						.getStreet());
				SearchUtils.addIlikeRestrictionI18n(crit, "city", address
						.getCity());
				SearchUtils.addEqRestriction(crit, "country", address
						.getCountry());
			}
		}
		// billig restrictions
		if (cExample.getBilling() != null) {
			...
			SearchUtils.addEqRestriction(crit, "frequency", billing
					.getFrequency());
			SearchUtils.addEqRestriction(crit, "is_billed_after", billing
					.getIsBilledAfter());
			SearchUtils.addEqRestriction(crit, "is_active", billing
					.getIsActive());
