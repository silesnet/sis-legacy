...
public class CustomerDAOHibernate
    ...
    public List<Customer> getAll() {
    	...
    	return (ArrayList<Customer>) getHibernateTemplate().findByCriteria(crit);
    }
...
//    	SearchUtils.addIlikeRestriction(crit, "public_id", cExample.getPublicId());
    	// first 8 digits (excluding '/') are export public_id, use this export public id instead of raw public id
	...
	public int getTotalCustomers(Country c) {
		...
		Integer total = c == null
			? (Integer) getHibernateTemplate().find("select count(*) from Customer").get(0)
			: (Integer) getHibernateTemplate().find("select count(*) from Customer c" +
					" where c.contact.address.country=?", c).get(0);
