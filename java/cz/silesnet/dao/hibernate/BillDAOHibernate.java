...
public class BillDAOHibernate extends HibernateDaoSupport implements BillDAO {
    ...
    public Bill get(String uuid) {
        List<Bill> bills = getHibernateTemplate().find("from Bill b where b.hashCode = ?", uuid);
    ...
    public List<Bill> getByCustomer(Customer c) {
        return getHibernateTemplate().find("from Bill b where b.customerId=? order by b.number asc", c.getId());
    }
    ...
    public Bill getToSend(Country country) {
        ...
        List<Bill> bills = getSession().createSQLQuery(sqlQuery).addEntity(Bill.class).list();
