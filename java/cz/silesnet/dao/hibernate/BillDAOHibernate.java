...
public class BillDAOHibernate
    ...
    public Bill get(Long billId) {
        Bill bill = (Bill) getHibernateTemplate()
                                   .get(Bill.class, billId);
        if (bill == null)
    ...
    public Bill get(String uuid) {
        List<Bill> bills = (ArrayList<Bill>) getHibernateTemplate()
                                   .find("from Bill b where b.hashCode = ?",
                    uuid);
...
//		crit.createCriteria("customer").addOrder(Order.asc("name"));
		...
		return (ArrayList<Bill>) getHibernateTemplate().findByCriteria(crit);
	}
	...
	public List<Bill> getBySentMail(Invoicing invoicing) {
		...
		crit.addOrder(Order.asc("number"));
		return (ArrayList<Bill>) getHibernateTemplate().findByCriteria(crit);
//		return (ArrayList<Bill>) getHibernateTemplate().find("from Bill b where b.isConfirmed=TRUE and b.isSent=TRUE and b.isDelivered=FALSE and b.isArchived=FALSE and b.customer.billing.deliverByMail=TRUE");
//		return (ArrayList<Bill>) getHibernateTemplate().find("from Bill b where b.isConfirmed=TRUE and b.isSent=TRUE and b.isDelivered=FALSE and b.isArchived=FALSE and b.deliverByMail=TRUE order by b.number asc");
	}

	public List<Bill> getByCustomer(Customer c) {
		return (ArrayList<Bill>) getHibernateTemplate().find("from Bill b where b.customerId=? order by b.number asc", c.getId());
	}
	...
	public List<Bill> getByNumber(String billNumber) {
		...
		return (ArrayList<Bill>) getHibernateTemplate().findByCriteria(billCrit);
	}
	...
	public Customer fetchCustomer(Bill bill) {
		Customer customer = (Customer) getHibernateTemplate().get(Customer.class, bill.getCustomerId());
		return customer;
	}
	...
	public int getCountByStatus(Invoicing invoicing, Boolean isConfirmed, Boolean isSent, Boolean isDelivered, Boolean isArchived, Boolean isSnail) {
		...
		return ((Integer) getHibernateTemplate().findByCriteria(crit).get(0)).intValue(); 
	}
	...
	public Bill getToSend(Country country) {
		...
		List<Bill> bills = (ArrayList<Bill>) getSession().createSQLQuery(sqlQuery).addEntity(Bill.class).list(); 
//		List<Bill> bills = (ArrayList<Bill>) getHibernateTemplate().findByCriteria(crit, 0, 1);
		if (bills.size() > 0)
			return bills.get(0);
		else
			return null;
	...
	public List<Invoicing> getInvoicings(Country country) {
		...
		return (ArrayList<Invoicing>) getHibernateTemplate().findByCriteria(crit);
	}

	public Invoicing getInvoicing(Long id) {
		Invoicing invoicing = (Invoicing) getHibernateTemplate().get(Invoicing.class, id);
        if (invoicing == null)
