...
public class NodeDAOHibernate extends HibernateDaoSupport implements NodeDAO {
    ...
    public Node getNodeByName(String nodeName) {
        ArrayList<Node> nodes = (ArrayList<Node>) getHibernateTemplate()
                                                          .find("from cz.silesnet.model.Node as node where node.name=?",
                    nodeName);
    ...
    public List<Node> getSubNodes(Long parentId) {
        return (ArrayList<Node>) getHibernateTemplate()
                                         .find("from cz.silesnet.model.Node as node where node.parentId=? order by node.name",
                parentId);
    }

    public List<Node> getSubNodesByDomain(Long parentId, Long domainLabelId) {
        return (ArrayList<Node>) getHibernateTemplate()
                                         .find("from cz.silesnet.model.Node as node where (node.parentId=?) and (node.domainLabel.id=?) order by node.name",
	...
	public List<Node> getByExample(Node example) {
		...
		if (example instanceof Wireless) {
			...
			SearchUtils.addEqRestriction(crit, "domain_lid", wireless.getDomain());
	...
	public List<Node> getAll() {
		return (ArrayList<Node>) getHibernateTemplate().find("from Node n order by n.name");
	}
}