 ...
 */
public class HistoryItemDAOHibernate
    extends HibernateDaoSupport
    implements HistoryItemDAO {
    ...
    public List<HistoryItem> getHistory(Long historyId) {
        return (ArrayList<HistoryItem>) getHibernateTemplate()
                                                .find("from cz.silesnet.model.HistoryItem as item where item.historyId=? order by item.timeStamp desc, item.id desc",
                historyId);
    }
    ...
    public Long getNewHistoryId() {
        ...
        Object result = getHibernateTemplate()
                                .find("select max(item.historyId) from cz.silesnet.model.HistoryItem as item)")
                                .get(0);
    ...
    public List<HistoryItem> getOpenLoginItems(String sessionId) {
        ...
        return (ArrayList<HistoryItem>) getHibernateTemplate()
                                                .find("from cz.silesnet.model.HistoryItem as item where (item.historyTypeLabel.id=?) and (item.oldValue=?) and (item.newValue=?) order by item.timeStamp desc",
                new Object[] {Long.valueOf(17), sessionId, "lastLogin.Online" });
    }

    /*
     * (non-Javadoc)
     *
     * @see cz.silesnet.dao.HistoryItemDAO#getUserHistory(cz.silesnet.model.User,
