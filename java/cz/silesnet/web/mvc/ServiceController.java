...
public class ServiceController
    ...
    public ModelAndView delete(HttpServletRequest request,
        ...
        MessagesUtils.setCodedSuccessMessage(request,
            "editService.deleteSuccess",
            new Object[] { command.getId(), command.getName() });
...
//        model.put("servicesList", smgr.getAllOrphans());
    ...
    protected Boolean isFormBackingObjectNew(HttpServletRequest request) {
        return (ServletRequestUtils.getLongParameter(request, "serviceId", 0) == 0)
        ? true : false;
    }
    ...
    protected Object formBackingObject(HttpServletRequest request)
        ...
        if (isFormBackingObjectNew(request)) {
            ...
            if ("oneTime".equals(ServletRequestUtils.getStringParameter(request, "formType", ""))) {
                s.setFrequency(Frequency.ONE_TIME);
            } else {
            	s.setFrequency(Frequency.MONTHLY);
            }
        } else
            s = cmgr.getService(ServletRequestUtils.getRequiredLongParameter(request,
                        "serviceId"));
        return s;
    }
    ...
    protected Map<String, Object> referenceData(HttpServletRequest request) {
        ...
        if (isOneTimeService(request)) {
	        ...
	        model.put("serviceFrequency", EnumSet.of(Frequency.MONTHLY, Frequency.ANNUAL));
        }
        ...
        model.put("scripts",
            new String[] { "safeSubmit.js", "calendar.js", "comboBox.js" });
    ...
    private boolean isOneTimeService(HttpServletRequest request) {
        ...
        if ("oneTime".equals(formType)) return true;
        if (!isFormBackingObjectNew(request)) {
        	Object o = null;
			try {
				// it is not nice to call formBackingObject() the second time
				// but it's already probably in the persistence cache so it should
    ...
    protected void validate(Object command, BindingResult bindingResult) {
    	...
    	if (service.getAdditionalName() == null || "".equals(service.getAdditionalName())) {
        ...
        if (!service.getPeriod().isValid())
			bindingResult.rejectValue("period.to", "editService.periodMishmash", "Period mishmash.");
	}
    ...
    protected Map referenceI18nFields(Object o) {
    	...
    	if (o instanceof Service) {
    		...
    		if (c != null && c.getContact().getAddress().getCountry() == Country.PL) {
