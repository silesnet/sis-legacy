...
public class Customer extends Entity implements Historic {
    ...
    private static final String[] sDiffExcludeFields = {
            "class$0", "serialVersionUID", "sHistoryTypeLabelId",
    ...
    public String getServicesInfo() {
    	...
    	if (getServices().size() == 0) return comm;
    	return getServices().size() > 1
    		? getServices().get(0).getShortInfo() + " (+…) " + comm
    		: getServices().get(0).getShortInfo() + " " + comm;
    }
