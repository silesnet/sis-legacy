...
/**
 * Concrete implementation of CustomerManager usig
 * CustomerDAO for CRUD operations.
 *
...
//        Customer formerCustomer = dao.load(customer.getId());
        ...
        Assert.isTrue(formerCustomer.getHistoryId()
                                        .equals(customer.getHistoryId()),
	...
	public Map<String, String> getOverview(Country c) {
		...
		case 0: 
			// overall, no country
			overview.put("overviewCustomers.totalPrice.CZK",
					f.format(total)
		...
		case 20:
			// PL
			overview.put("overviewCustomers.totalPrice.CZK",
					f.format(totalPL_CZK)
					+ " " + MessagesUtils.getMessage("money.label.cz"));
			overview.put("overviewCustomers.totalPrice.PLN",
					f.format(totalPL)
	...
	public void exportCusotmersToWinDuo(List<Customer> customers, PrintWriter writer) {
		...
		for (Customer c : customers) {
			...
			writer.printf("%s\t%s\t%s\t%s\t", sanate(c.getExportPublicId()), sanate(c.getDIC()),
					c.getName(), sanate(c.getSupplementaryName()));
			// Street, City, PSC, Country
			writer.printf("%s\t%s\t%s\t%s\t", c.getContact().getAddress().getStreet(),
					c.getContact().getAddress().getCity(), sanate(c.getContact().getAddress().getPostalCode()),
					MessagesUtils.getMessage(c.getContact().getAddress().getCountry().getName(), new Locale("cs")));
			// Account, Bank, Person
	...
	public void insertService(Service service) {
		if (service.getCustomerId() != null) {
			...
			// need to evic koz customer is transient all chages are auto persisted without audit
			dao.evict(c);
	...
	public List<Customer> getByExample(Customer customer, Service service) {
		...
		if (customers == null) {
			...
			customers = (List<Customer>) ListUtils.intersection(customers, getCustomers(services));
		}
		return customers;
	}
	...
	public void deactivateCandidates() {
		...
		for (Customer customer : customers) {
			if (customer.isDeactivateCandidate(due)) {
				// detach the customer from hibernate session before change, we are in transaction
	...
	public void updateSymbol(Customer customer) {
		if (customer.getSymbol() == null || "".equals(customer.getSymbol())) {
			...
			if (Country.PL.equals(customer.getContact().getAddress().getCountry())) {
				...
				String shortName = StringUtils.replaceChars((customer.getName() + "XXX").substring(0, 3), SearchUtils.getFromChars(), SearchUtils.getToChars());
	...
	public void exportCusotmersToInsert(List<Customer> customers, PrintWriter writer) {
		...
		for (Customer customer : customers) {
			...
			// symbol, names
			writer.printf("2,\"%s\",\"%s\",\"%s\",", escapeQuotes(customer.getSymbol()), escapeQuotes(customerName), escapeQuotes(customerLongName));
			// address, dic, ico
			Address a = customer.getContact().getAddress();
			writer.printf("\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",,,\"%s\",,,,,,,,,,,,,,,", a.getCity(), a.getPostalCode(), a.getStreet(), customer.getDIC(), customer.getPublicId(), customer.getContact().getPhone(), customer.getContact().getEmail());
			// trailer
