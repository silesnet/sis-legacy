...
public class BillingManagerImpl implements BillingManager {
    ...
    public List<Bill> getByStatus(Invoicing invoicing, Boolean isConfirmed, Boolean isSent, Boolean isDelivered,
            Boolean isArchived) {
    ...
    public List<Bill> generateAll(Invoicing invoicing, List<Customer> customers, Date due, String lastInvoiceNo) {
        ...
        while (cIter.hasNext()) {
            ...
            if (!BillingStatus.INVOICE.equals(customer.getBilling().getStatus())) {
                ...
                hmgr.insertSystemBillingAudit(invoicing, customer, "mainBilling.msg.billinDisabled",
            ...
            try {
                ...
                hmgr.insertSystemBillingAudit(invoicing, customer, "mainBilling.msg.illegalArgument",
            ...
            if (newInvoice == null) {
                ...
                hmgr.insertSystemBillingAudit(invoicing, customer, "mainBilling.msg.noInvoice",
                        "mainBilling.status.skipped");
                cIter.remove();
                skipped++;
                continue;
            ...
            if (newInvoice.getTotalPrice() == 0) {
                ...
                if (!oneTimePresent) {
                    ...
                    hmgr.insertSystemBillingAudit(invoicing, customer, "mainBilling.msg.zeroInvoice",
                            "mainBilling.status.skipped");
                    cIter.remove();
                    skipped++;
                    continue;
            ...
            if (newInvoice.getTotalPrice() < 0) {
                ...
                hmgr.insertSystemBillingAudit(invoicing, customer, "mainBilling.msg.negativeInvoice",
                        "mainBilling.status.skipped");
                cIter.remove();
                skipped++;
                continue;
    ...
    public Bill generate(Customer customer, Date due, String number) {
        ...
        for (Service service : customer.getServices()) {
            ...
            BillItem invoiceItem = new BillItem(service
                    .getBillItemText(customer.getContact().getAddress().getCountry()), itemAmount, service.getPrice());
     ...
     *            Customer.billing
     * @param due
     *            date, the first of the due date month will be used (eg
    ...
    public Bill send(Bill bill, MutableInt emailedCounter) {
        ...
        if (c.getBilling().getDeliverByEmail()) {
            try {
                ...
                hmgr.insertSystemBillingAudit(invoicing, c, "mainBilling.msg.emailAddressError",
    ...
    public void reSendAll(Invoicing invoicing, List<Bill> bills) {
        ...
        for (Bill bill : bills) {
            ...
            if (customer.getBilling().getDeliverByEmail()) {
                try {
                    ...
                    // bill parse email error, autid it
                    if (invoicing != null)
                        hmgr.insertSystemBillingAudit(invoicing, customer, "mainBilling.msg.emailAddressError",
                                "mainBilling.status.skipped");
                    log.warn("Bill email parse exception: " + customer.getName());
                } catch (MailException e) {
                    // bill send email error, autid it
                    if (invoicing != null)
                        hmgr.insertSystemBillingAudit(invoicing, customer, "mainBilling.msg.emailSendingError",
                                "mainBilling.status.skipped");
                    log.warn("Bill email sending exception: " + customer.getName());
                }
                // due to anti-spam protection wait for certain time
        ...
        if (invoicing != null)
            hmgr.insertSystemBillingAudit(invoicing, null, "mainBilling.msg.reSendingFinished", emailedCounter + "/"
                    + bills.size() + " ***");
    }
    ...
    public void email(final Bill bill) throws MailException {
        ...
        final String emailFrom = Country.PL.equals(c.getContact().getAddress().getCountry()) ? emailFromAddressPl
                : emailFromAddressCs;
        MimeMessagePreparator msgPreparator = new MimeMessagePreparator() {
            public void prepare(MimeMessage msg) throws MessagingException {
                ...
                email.setSubject(MessagesUtils.getMessage("billEmail.subject", new Object[] { bill.getNumber(),
                        bill.getPeriod().getPeriodString() }, locale));
                StringBuffer text = new StringBuffer();
                text.append(MessagesUtils.getMessage("billEmail.text.header", bill.getPeriod().getPeriodString(),
                        locale));
                ...
                if (c.getBilling().getDeliverSigned() && Country.CZ.equals(c.getContact().getAddress().getCountry())) {
                    ...
                    try {
                        ...
                        if (invoicing != null) {
                            hmgr.insertSystemBillingAudit(invoicing, c, "mainBilling.msg.emailSignError",
    ...
    private void emailReminder(final Bill bill) throws MailException {
        ...
        MimeMessagePreparator msgPreparator = new MimeMessagePreparator() {
            public void prepare(MimeMessage msg) throws MessagingException {
                ...
                email.getMimeMessage().addHeader("X-Priority", "1");
                email.getMimeMessage().addHeader("X-MSMail-Priority", "High");
                email.setSubject(MessagesUtils.getMessage("billEmail.reminder.subject", new Object[] {
                        bill.getNumber(), bill.getPeriod().getPeriodString() }, locale));
                StringBuffer text = new StringBuffer();
                text.append(MessagesUtils.getMessage("billEmail.reminder.text.header", bill.getPeriod()
    ...
    public void exportAllToWinduo(Invoicing invoicing, List<Bill> bills, PrintWriter writer) {
        ...
        for (Iterator<Bill> iter = bills.iterator(); iter.hasNext();) {
            ...
            // dates
            writer.printf("\t%s\t%s\t%s\t%s\t", dFmt.format(b.getBillingDate()), dFmt.format(b.getBillingDate()), dFmt
                    .format(b.getPurgeDate()), dFmt.format(b.getBillingDate()));
    ...
    public void invoiceAllCustomers(Invoicing invoicing, Date due, String invoiceNumberingBase) {
        ...
        List<Customer> customers = (List<Customer>) CollectionUtils.collect(bills, new BeanToPropertyValueTransformer(
                "invoicedCustomer"));
    ...
    public int getCountByStatus(Invoicing invoicing, Boolean isConfirmed, Boolean isSent, Boolean isDelivered,
            Boolean isArchived, Boolean isSnail) {
    ...
    public void invoice(Invoicing invoicing) {
        ...
        List<Bill> bills = generateAll(invoicing, customersToInvoice, invoicing.getInvoicingDate(), invoicing
                .getNumberingBase());
    ...
    public void exportAllToInsert(Invoicing invoicing, List<Bill> bills, PrintWriter writer) {
        ...
        for (Iterator<Bill> iter = bills.iterator(); iter.hasNext();) {
            ...
            writer.printf("%s,%s,%s,,%d,1,%s,", spec1, invoicingTimeStamp, invoicingTimeStamp, b.getItems().size(),
                    spec2);
            // amounts
            writer.printf("%.4f,%.4f,%.4f,0.0000,,0.0000,,", new Float(b.getTotalPrice()), b.getBillVat(), b
            ...
            for (BillItem bi : b.getItems()) {
                // line prefix
                writer.printf("%d,2,,1,0,0,1,0.0000,0.0000,%s,1.0000,1.0000,0.0000,", ++cnt,
                        bi.getIsDisplayUnit() ? '"' + specUnit + '"' : "");
                float lineNet = new Float(bi.getLinePrice());
                // line amounts
                writer.printf("%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,0.0000,,", lineNet, bi.getLinePriceVat(), new Float(b
                        .getVat()), lineNet, bi.getLineVat(), bi.getLinePriceVat());
                // line text
                if (bi.getIsDisplayUnit()) {
