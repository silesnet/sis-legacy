...
public class NavigationUtils {
    ...
    public static String getReturnUrl(HttpServletRequest request, String fallbackUrl) {
        StringBuffer returnUrl = new StringBuffer(request.getContextPath());
        // try to get from session _navUrlStack first
        Stack<String> urlStack = (Stack<String>) request.getSession().getAttribute("_navUrlStack");
    ...
    public static boolean isTablePagination(HttpServletRequest request) {
    	Boolean forceRefresh = (Boolean) request.getSession().getAttribute("forceRefreshList");
    	...
    	if (forceRefresh != null && forceRefresh) {
    		...
    		return WebUtils.getParametersStartingWith(request, "d-").size() == 0 ? false : true;
	}
    
    public static void setForceListRefresh(HttpServletRequest request, boolean value) {
