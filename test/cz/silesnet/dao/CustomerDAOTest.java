...
public class CustomerDAOTest
    ...
    public void testCRUD() {
        ...
        assertTrue("Modified Street".equals(c.getContact().getAddress().getStreet()));
        assertTrue("admin@gymopol.cz".equals(c.getContact().getEmail()));
        assertTrue("Modified Service Info String".equals(c.getServices().get(0).getInfo()));
        assertTrue(c.getBilling().getFrequency().equals(Frequency.DAILY));

        log.debug("Retrieved updated customer: " + c);

        // does not work with non transactional session, see HibernateTemplate.enableFilter()
