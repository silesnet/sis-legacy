...
public class CustomerManagerImpl implements CustomerManager {
	...
	public Map<String, String> getOverview(Country c) {
		...
		overview.put("overviewCustomers.totalCustomers", f.format(dao
				.getTotalCustomers(c)));
		// total download
		overview.put("overviewCustomers.totalDownload", f.format(sDao
				.getTotalDownload(c))
				+ " " + MessagesUtils.getMessage("netSpeed.label"));
		// total upload
		overview.put("overviewCustomers.totalUpload", f.format(sDao
				.getTotalUpload(c))
				+ " " + MessagesUtils.getMessage("netSpeed.label"));
		...
		case 20:
			// PL
			overview.put("overviewCustomers.totalPrice.CZK", f
					.format(totalPL_CZK)
					+ " " + MessagesUtils.getMessage("money.label.cz"));
			overview.put("overviewCustomers.totalPrice.PLN", f.format(totalPL)
					+ " " + MessagesUtils.getMessage("money.label.pl"));
			overview.put("exchangeRate.PLN_CZK", Double.valueOf(ratePLN_CZK)
					.toString());
			break;
		}
		return overview;
